...
//        }
     ...
     * 恢复极光推送服务
     */
   /* public static void setJPushOn(Context context) {
        if (JPushInterface.isPushStopped(context)) {// 如果是关闭状态，就恢复
            Log.w("JPush", "开启极光");
    ...
    public static void saveLogcat(String fileName, String write) {
        ...
        if (status.equals(Environment.MEDIA_MOUNTED)) {
            try {
                String pathName = Utils.getApp().getExternalFilesDir("/").getPath() + "/ctclient_log";
    ...
    /*public static void saveCrashLogcat(MyCrashHandler instance, Throwable ex, Context context) {
        String status = Environment.getExternalStorageState();
        if (status.equals(Environment.MEDIA_MOUNTED)) {
            if (instance != null) {
                try {
                    ...
                    File path = new File(filename + "/" + "crash_log.txt");
                    FileOutputStream fileOutputStream = new FileOutputStream(
                            path, true);

                    SimpleDateFormat formatter = new SimpleDateFormat("yyyy年MM月dd日 HH:mm:ss ");
                    Date curDate = new Date(System.currentTimeMillis());// 获取当前时间
                    String phoneInformation = getPhoneInformation(context);//获取手机相关信息
                    String nowTime = phoneInformation + formatter.format(curDate);

                    String writeToFile = nowTime + ":\n";
                    byte[] bytes = writeToFile.getBytes();
...
*/
    public static String getPhoneInformation(Context context) {
        ...
        return "手机型号:" + UtilPhoneParam.getPhoneModel() + "\n" + "手机系统:" + UtilPhoneParam.getPhoneSystemVersion() + "\n"
                + "屏幕分辨率:" + UtilPhoneParam.screenHeight + "x" + UtilPhoneParam.screenWidth + "\n" + "手机内存:" + UtilPhoneParam.getMemoryInfo() + "\n"
                + "单应用内存大小:" + memClass + "M\n" + "应用版本号:" + UtilBaseApp.getCurAppVer(context) + "\n"
                + "应用版本名:" + UtilBaseApp.getCurAppVerName() + "\n";
    }
     ...
     * @author zhuofq
     */
    public static String getCurAppVerName() {
        String verName = "1.0.0";
        try {
            verName = Utils.getApp().getPackageManager().getPackageInfo(
   ...
   /* public static String getChannelId(Context context) {
        ...
        if (UtilText.isEmptyOrNull(channel)) {
            channel = getChannel(context);
            if (UtilText.isEmptyOrNull(channel)) {
                channel = getChannelIdOld(context);
            }
        }
        return channel;
    }*/
